...
while esperar_syn:
    ...
    #paquete_inesperado = len(pkt_capturado) != 0 and pkt_capturado[0][TCP].ack != 0 and pkt_capturado[0][TCP].seq != seq_ack[0] 
    #nunca me puede llegar uno inesperado
    ...
    if paquete_perdido:
        ...
        # Medio innecesario (preguntar). Nunca iria a pasar porque siempre la secuencia del ack te tiene que llegar en 0. 
        if pkt_capturado[0][TCP].ack == 0:
            seq_ack[1] = pkt_capturado[0][TCP].seq + 1
            enviar_respuesta(pkt_capturado, seq_ack[0])
            esperar_syn = False
        elif pkt_capturado[0][TCP].ack != 0 and pkt_capturado[0][TCP].seq == seq_ack[1]:
            seq_ack[1] += 1
...
while enviar_syn_ack:
    ...
    if paquete_perdido: 
        ...
        seq_ack[0] += 1
        seq_ack[1] = pkt_capturado[0][TCP].seq + 1
        # los seqs y acks pueden aumentar siempre en 1 o dependen de lo que captura?
